...
$.validationEngine = {
	defaultSetting : function(caller) {		// NOT GENERALLY USED, NEEDED FOR THE API, DO NOT TOUCH
		if($.validationEngineLanguage){				
			...
			$.validationEngine.debug("some debug info...");
		}	
		settings = {
			allrules:allRules,
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		for (i=0; i<rules.length;i++){
			...
			break;
			case "sp_char": 
				_sp_char(caller,rules,i);
			break;
			case "sp_char_space":
				_sp_char_space(caller,rules,i);
			break;
			case "sp_char_userName":
				_sp_char_userName(caller,rules,i);
			break;
			case "custom": 
		...
		function _sp_char(caller,rules,position){   //special characters
		...
		function _sp_char_space(caller,rules,position){   //special characters no space
			customRule = rules[position];
			pattern = eval($.validationEngine.settings.allrules[customRule].regex);
			if(pattern.test($(caller).attr('value'))){
				$.validationEngine.isError = true;
		...
		function _sp_char_userName(caller,rules,position){   //special characters no hanzi
			customRule = rules[position];
			pattern = eval($.validationEngine.settings.allrules[customRule].regex);
			if(pattern.test($(caller).attr('value'))){
				$.validationEngine.isError = true;
		...
		function _funcCall(caller,rules,position){  		// VALIDATE CUSTOM FUNCTIONS OUTSIDE OF THE ENGINE SCOPE
			customRule = rules[position+1];
			funce = $.validationEngine.settings.allrules[customRule].name;
			var fn = window[funce];
		...
		function _ajax(caller,rules,position){				 // VALIDATE AJAX RULES
			...
			if(!ajaxisError){
				$.ajax({
					...
					cache: false,
					error: function(transport){ $.validationEngine.debug("数据提交错误: "+data.status+" "+transport) },
					success: function(data){					// GET SUCCESS DATA RETURN JSON
						...
						existInarray = false;
			 			 ...
			 			 if(data.success == true){
							...
							//5秒后清除弹出框
							setTimeout(function(){
								$("." + $(caller).attr("id") + "formError").remove();
							}, 3000);
						 	_checkInArray(true);
	...
	buildPrompt : function(caller,promptText,type,ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		...
		// 调整提示框的位置（位于文本框之后）
		$(divFormError).css({
			"top":calculatedPosition.callerTopPosition+20+'px',
	...
	submitValidation : function(caller) {					// FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
		var stopForm = false;
		$.validationEngine.ajaxValid = true;
		var toValidateSize = $(caller).find("[class*=onsubmit]").size();
		//validate=>submit
		$(caller).find("[class*=onsubmit]").each(function(){
