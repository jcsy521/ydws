...
public class PushAction extends ActionSupport {
	...
	public String execute() {
		...
		logger.info("[ANDROIDPUSH] push request: user: " + uid + ", content: \n"
				+ body);
		if (user == null) {
			status = 1;
			message = "uid does not exist!";
			logger.error("[ANDROIDPUSH] user: " + uid + ", does not exist!");
			// } else if (!user.getPassword().equals(key)) {
			// status = 2;
			// message = "Key is error!";
		} else {
			pusher.sentMessage(uid, body);
			logger.info("[ANDROIDPUSH] user: " + uid + ", send message!");
		}
		logger.info("[ANDROIDPUSH] push response: user: " + uid + ", message: "
				+ message);
		return SUCCESS;
	}
