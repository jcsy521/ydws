...
public class AccountAction extends ActionSupport {
	...
	public String create() {
		...
		logger.info("[ANDROIDPUSH] create account request: user: " + uid);
		if (user != null) {
			status = 1;
			message = "Has been registered!";
			key = "e11e7e3e21180fd";
			logger.info("[ANDROIDPUSH] user: " + uid + " has been registered");
		} else {
			// key = MD5Util.getMD5(
			...
			if (OpenfireAdaptor.createAccount(uid, key)) {
				...
				key = "e11e7e3e21180fd";
				logger.info("[ANDROIDPUSH] user: " + uid + ", register success");
				// user = new DPUser();
				...
				key = "";
				logger.error("[ANDROIDPUSH] user: " + uid + ", register failed");
			}
		}
		logger.info("[ANDROIDPUSH] create account response: user: " + uid
				+ ", message: " + message);
