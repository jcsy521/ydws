...
window.dlf.fn_switchCar = function(n_tid, obj_currentItem) {
	...
	/* todo 终端挂件
	if ( n_keys_num ) {
		dlf.fn_showOrHideLocation(n_keys_num);
	}
	*/
	// 向后台发送切换请求
		...
		if ( data.status == 0 ) {
			...
			var obj_toCentInterval = setInterval(function() {
				...
				if ( obj_tempMarker ) {
					...
					obj_tempMarker.openInfoWindow(obj_tempMarker.selfInfoWindow); // 显示吹出框
...
window.dlf.fn_updateInfoData = function(obj_carInfo, str_type) {
	...
	// 终端别名赋值
	if ( !str_alias ) {
		str_alias += $('#carList a[tid='+str_tid+']').siblings('span').html();
	}	
	// 存储车辆信息
	if ( obj_tempVal ) { // 追加
