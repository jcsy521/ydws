...
function fn_initEventTimeControl(str_who) {
	...
	obj_stTime.unbind('click').bind('click', function() {	// 初始化起始时间，并做事件关联 maxDate: '#F{$dp.$D(\''+str_inputEndTime+'\')}',minDate: '#F{$dp.$D(\''+str_inputStartTime+'\')}', // delete in 2013.04.10
		WdatePicker({el: str_inputStartTime, lang: 'zh-cn', dateFmt: str_timepickerFormat, readOnly: true, isShowClear: false,  qsEnabled: false, autoPickDate: false,
		onpicked: function() {
	...
	obj_endTime.unbind('click').bind('click', function() {	// 初始化结束时间，并做事件关联
		WdatePicker({el: str_inputEndTime,lang: 'zh-cn', dateFmt: str_timepickerFormat, readOnly: true, isShowClear: false, qsEnabled: false, autoPickDate: false, onpicked: function() {
				//if ( !dlf.fn_userType() ) {	// 如果是个人用户 有时间限制
