...
class OnlineHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            ...
            logging.exception("[UWEB] cid:%s, oid:%s search online statistic "
                              "  data format illegal. Exception: %s",
        ...
        try:
            ...
            res = self.db.query("SELECT online_num, offline_num, "
                                "  time, (online_num + offline_num) AS total_num"
                                "  FROM T_ONLINE_STATISTIC"
                                "  WHERE time BETWEEN %s AND %s"
                                "  AND cid = %s"
                                "  ORDER BY time DESC"
            ...
            logging.exception("[UWEB] cid:%s, oid:%s search online "
                              "  statistic failed. Exception: %s",
                              self.current_user.cid, self.current_user.oid, e.args)
...
class OnlineDownloadHandler(OnlineHandler):
    ...
    def get(self):
        ...
        if r:
            ...
            logging.exception("[UWEB] online statistic export excel failed, "
