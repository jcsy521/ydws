...
class DoFile(object):
    ...
    def dofile(self):
        ...
        for file in self.filelist:
            try:
                ...
                if conf_logmodel == "java":
                    ...
                    logging.exception("Logmodels only can be celery, jave, python,"
                                      " check the monitor.conf again!")
                # after the handler, remove the file
                os.remove(_conf_file + "error.log")
                logging.info(conf_sessionname + "deal with file success")
            except Exception as e:
                logging.exception("Handle files fail. Exception: %s", 
                                  e.args)
        ...
        try:
            ...
            logging.exception("Clean up old data failed! Exception: %s", 
                              e.args)
