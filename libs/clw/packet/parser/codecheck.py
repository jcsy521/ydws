    ...
    def parse_head(self, packets):
        ...
        if packets.startswith('[') and packets.endswith(']'):
            ...
            for packet in packet_list:
                ...
                p_info = packet.split(',') 
                keys = ['timestamp', 'dev_type', 'version', 'dev_id', 'type']
                for i, key in enumerate(keys):
                    packet_info.head[key] = p_info[i]
                timestamp = packet_info.head.timestamp if packet_info.head.timestamp else time.strftime("%Y-%m-%d %H:%M:%S")
                packet_info.head.timestamp = int(time.mktime(time.strptime(timestamp, '%Y-%m-%d %H:%M:%S'))) * 1000
                packet_info.body = p_info[5:]
