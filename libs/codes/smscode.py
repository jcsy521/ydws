...
class SMSCode(object):
    ...
    SMS_TRACKER_POWERLOW_NOLOC = u"尊敬的客户：您的定位器“%s”电量不足，当前电量为%d%%，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    ...
    SMS_POWERLOW_OFF_NOLOC = u"尊敬的客户：您的定位器“%s”电量过低，即将关机，为了安全请及时充电并保持开机状态。地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    SMS_POWERFULL = u"尊敬的客户：您的定位器“%s”已经充电完成，请拔下电源。"
    SMS_POWERDOWN = u"尊敬的客户：您的定位器“%s”检测到电动车蓄电池断电。地址：%s，时间：%s。"
    SMS_POWERDOWN_NOLOC = u"尊敬的客户：您的定位器“%s”检测到电动车蓄电池断电。地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    SMS_ILLEGALMOVE = u"尊敬的客户：您的车辆“%s”发生移动，地址：%s，时间：%s。如需取消短信提醒，请执行撤防操作。"
    SMS_ILLEGALMOVE_NOLOC = u"尊敬的客户：您的车辆“%s”发生移动，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。如需取消短信提醒，请执行撤防操作。"
    SMS_ILLEGALSHAKE= u"尊敬的客户：您的车辆“%s”发生震动，请关注车辆状态，地址：%s，时间：%s。如需取消短信提醒，请执行撤防操作。"
    SMS_ILLEGALSHAKE_NOLOC = u"尊敬的客户：您的车辆“%s”发生震动，请关注车辆状态，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。如需取消短信提醒，请执行撤防操作。"
    SMS_HEARTBEAT_LOST = u"尊敬的客户：您的定位器“%s”与平台通讯可能出现异常，请检查定位器是否处于正常状态，时间：%s。"
    SMS_SOS = u"您的定位器“%s”发起的应急救援请求，【盛宇百祺定位系统】平台已收到，地址：%s，时间：%s。"
    SMS_SOS_NOLOC = u"您的定位器“%s”发起的应急救援请求，【盛宇百祺定位系统】平台已收到，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    SMS_SOS_OWNER = u"您的定位器“%s”发起应急救援请求，已通知%s，地址：%s，时间：%s。"
    SMS_SOS_OWNER_NOLOC = u"您的定位器“%s”发起应急救援请求，已通知%s，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    SMS_SOS_WHITE = u"您的定位器“%s”发起应急救援请求，当前所在位置：%s，时间：%s。"
    SMS_SOS_WHITE_NOLOC = u"您的定位器“%s”发起应急救援请求，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
    SMS_LOGIN_REMIND = u"尊敬的客户：您于%s通过%s登陆【盛宇百祺定位系统】平台，车主号码为%s，所关联的定位器为“%s”。"
    SMS_CHARGE = u"尊敬的客户：您的定位器“%s”当前帐户信息：%s"
    SMS_CHARGE_REMIND = u"尊敬的客户：为了不影响盛宇百祺定位系统业务的正常使用，请您及时为您的盛宇百祺定位系统账号：%s进行充值，如已经缴费或者已经合并账号缴费请不用理会此短信。【盛宇百祺定位系统】"
    SMS_SERVICE_STOP = u"尊敬的客户：您的定位器“%s”已经停止服务。"
    SMS_RETRIEVE_PASSWORD = u"尊敬的客户：您刚才使用了【盛宇百祺定位系统】密码找回功能，新的登陆密码为：%s，请妥善保管。如需修改密码，请登录【盛宇百祺定位系统】网站或手机客户端进行修改。" 
    SMS_REG = u"尊敬的客户：您的【盛宇百祺定位系统】验证码为：%s，请5分钟内进行激活。若非您本人或授权操作，请联系【盛宇百祺定位系统】客服。" 
    ...
    SMS_JH_FAILED = u"激活失败，请确认手机号是否正确，或联系【盛宇百祺定位系统】客服。"
    ...
    SMS_TID_EXIST = u"尊敬的客户：该定位器序列号%s冲突，请联系【盛宇百祺定位系统】客服。"
    ...
    SMS_MOBILE_NOT_WHITELIST = u"尊敬的客户：您的定位器号码%s不是指定盛宇百祺定位系统号码，请联系【盛宇百祺定位系统】客服。"
    SMS_DELETE_TERMINAL = u"尊敬的客户：您的定位器号码%s已经解绑。如非本人操作，请联系【盛宇百祺定位系统】客服。"
    SMS_UNUSUAL_ACTIVATE = u"尊敬的客户：该定位器%s已经被使用，原车主解绑后方可使用。"


    #SMS_DOWNLOAD_REMIND = u"尊敬的客户：请手机登录中山无线城市手机网站 gd.wxcs.cn/zs 进入‘下载’-‘盛宇百祺定位系统’ 下载安装【盛宇百祺定位系统】客户端。"
    SMS_DOWNLOAD_REMIND = u"尊敬的客户：请手机登录 www.pinganbb.com 下载安装【盛宇百祺定位系统】客户端。"

    SMS_NEW_OPERATOR = u"尊敬的客户：您已经成为盛宇百祺定位系统集团用户管理员，平台网址：%s，用户名：%s，密码：%s"
    ...
    SMS_REGION_ENTER_NOLOC = u"尊敬的客户：您的定位器“%s”已进入电子围栏“%s”，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态， 时间：%s。"
    SMS_REGION_OUT = u"尊敬的客户：您的定位器“%s”已离开电子围栏“%s”，地址：%s，时间：%s。"
    SMS_REGION_OUT_NOLOC = u"尊敬的客户：您的定位器“%s”已离开电子围栏“%s”，地址：因定位信号弱，当前暂时无法定位，请关注车辆状态，时间：%s。"
